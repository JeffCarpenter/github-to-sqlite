...
def pull_requests(
    ...
    if load:
        repo_full = utils.fetch_repo(repo, token)
        utils.save_repo(db, repo_full)
        utils.save_pull_requests(db, json.load(open(load)), repo_full)
    elif search:
        _save_searched_prs(db, token, search)
    else:
        repos = (
            itertools.chain.from_iterable(
                utils.fetch_all_repos(token=token, org=org_name) for org_name in orgs
            ) if orgs else [utils.fetch_repo(repo, token)]
        )
        for repo_full in repos:
            utils.save_repo(db, repo_full)
            prs = utils.fetch_pull_requests(repo_full["full_name"], state, token, pull_request_ids)
            utils.save_pull_requests(db, prs, repo_full)
...
def starred(
    ...
    stars = json.load(open(load)) if load else utils.fetch_all_starred(username, token)
    user = utils.fetch_user(username, token) if username else utils.fetch_user(token=token)
...
def stargazers(
    ...
    for repo in repos:
        ...
        utils.save_stargazers(db, repo_id, utils.fetch_stargazers(repo, token))
...
def repos(
    ...
    if load:
        ...
        for full_name in repo:
            ...
            _save_repo_readme(db, token, repo_id, full_name, readme, readme_html)
    else:
        for username in usernames or [None]:
            for repo_item in utils.fetch_all_repos(username, token):
                ...
                _save_repo_readme(db, token, repo_id, repo_item["full_name"], readme, readme_html)
...
def releases(
    ...
    for i, repo in enumerate(repos):
        ...
        repo_full = utils.fetch_repo(repo, token)
        utils.save_repo(db, repo_full)
        utils.save_releases(db, utils.fetch_releases(repo, token), repo_full["id"])
...
def tags(
    ...
    for i, repo in enumerate(repos):
        ...
        repo_full = utils.fetch_repo(repo, token)
        utils.save_repo(db, repo_full)
        utils.save_tags(db, utils.fetch_tags(repo, token), repo_full["id"])
...
def contributors(
    ...
    for repo in repos:
        repo_full = utils.fetch_repo(repo, token)
        utils.save_repo(db, repo_full)
        utils.save_contributors(db, utils.fetch_contributors(repo, token), repo_full["id"])
        time.sleep(1)
...
def commits(
    ...
    stop_when = None if all else _make_stop_when(db)
    
    for repo in repos:
        repo_full = utils.fetch_repo(repo, token)
        utils.save_repo(db, repo_full)
        utils.save_commits(db, utils.fetch_commits(repo, token, stop_when), repo_full["id"])
        time.sleep(1)
...
def scrape_dependents(
    ...
    for repo in repos:
        ...
        for dependent_repo in utils.scrape_dependents(repo, verbose):
            dependent_id = _get_or_fetch_repo_id(db, token, dependent_repo)
            _insert_dependent_if_new(db, repo_full["id"], dependent_id)
...
def workflows(
    ...
    for repo in repos:
        ...
        for filename, content in utils.fetch_workflows(token, full_repo["full_name"]).items():
