...
@click.option(
    ...
    "patterns",
    multiple=True,
    help="Additional glob patterns to search for build files",
)
@click.option(
    "-a",
    "--auth",
...
def starred_embeddings(db_path, model, force, verbose, patterns, auth):
    """Generate embeddings for repositories starred by the user."""
    token = load_token(auth)
    db = sqlite_utils.Database(db_path)
    utils.generate_starred_embeddings(
        db,
        token,
        model_name=model,
        force=force,
