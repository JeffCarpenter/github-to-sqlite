    ...
    if shutil.which("fd"):
        for pattern in BUILD_PATTERNS:
            ...
            for line in result.stdout.splitlines():
                line = line.strip()
                if line and "/.git/" not in line and "/node_modules/" not in line:
        ...
        for pattern in BUILD_PATTERNS:
            for full in pathlib.Path(path).rglob(pattern):
                if (
                    full.is_file()
                    and ".git" not in full.parts
                    and "node_modules" not in full.parts
                ):
                    found.append(
                        os.path.relpath(os.path.normpath(str(full)), path)
                    )
