...
from github_to_sqlite import embedding_utils
...
def test_maybe_load_sqlite_vec(monkeypatch):
    ...
    embedding_utils._SQLITE_VEC_CACHE.clear()
    assert embedding_utils._maybe_load_sqlite_vec(db) is False

    # Module with load that succeeds -> True
    dummy = type("M", (), {"load": lambda conn: None})
    monkeypatch.setitem(sys.modules, "sqlite_vec", dummy)
    embedding_utils._SQLITE_VEC_CACHE.clear()
    assert embedding_utils._maybe_load_sqlite_vec(db) is True
    embedding_utils._SQLITE_VEC_CACHE.clear()
